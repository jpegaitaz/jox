...
def _parse_chrome_args() -> List[str]:
    """
    Read optional extra Chrome flags from CHROME_ARGS
    (space-separated, e.g. '--lang=en-US --window-size=1920,1080').
    """
    raw = os.getenv("CHROME_ARGS", "")
    # simple split is fine for flags; ignore empties
...
class _ChromeCfg:
    ...
    chromedriver_path: Optional[str] = field(
        default_factory=lambda: os.getenv("CHROMEDRIVER") or os.getenv("CHROMEDRIVER_PATH")
    )
    user_agent: Optional[str] = field(default_factory=lambda: os.getenv("USER_AGENT"))
    browser_args: List[str] = field(default_factory=_parse_chrome_args)
...
class _Settings:
    ...
    is_interactive: bool = field(
        default_factory=lambda: bool(getattr(sys.stdin, "isatty", lambda: False)())
